...
class AnalysisService extends Service {
    ...
    async getAnalysisOneList(appId, markuser) {
        return (await this.app.models.WebEnvironment(appId).find({ mark_user: markuser }).read('sp').sort({ create_time: 1 }).exec()) || {};
    }
    ...
    async getDbTopAnalysis(appId, beginTime, endTime) {
        const data = await this.ctx.model.Web.WebStatis.findOne({ app_id: appId, create_time: { $gte: new Date(beginTime), $lte: new Date(endTime) } })
            .read('sp')
            .exec();
        if (data) return data;
        // 不存在则储存
