...
class PvuvipService extends Service {
    ...
    async getHistoryPvUvIplist(appId) {
        ...
        return await this.ctx.model.Web.WebPvuvip.find(query).read('sp').sort({ create_time: -1 }).limit(5).exec();
    }
    // 查询某日概况
    ...
    async uv(appId, querydata) {
        return this.app.models
            ...
            .aggregate([{ $match: querydata }, { $project: { mark_uv: true } }, { $group: { _id: '$mark_uv' } }, { $group: { _id: null, count: { $sum: 1 } } }])
            .read('sp')
            .exec();
    }
    // ip
    async ip(appId, querydata) {
        return this.app.models
            .WebEnvironment(appId)
            .aggregate([{ $match: querydata }, { $project: { ip: true } }, { $group: { _id: '$ip' } }, { $group: { _id: null, count: { $sum: 1 } } }])
            .read('sp')
            .exec();
    }
    // user
    async user(appId, querydata) {
        return this.app.models
            .WebEnvironment(appId)
            .aggregate([{ $match: querydata }, { $project: { mark_user: true } }, { $group: { _id: '$mark_user' } }, { $group: { _id: null, count: { $sum: 1 } } }])
            .read('sp')
            .exec();
    }
    // 跳出率
    ...
    async flow(appId, querydata) {
        ...
        const pageflow = Promise.resolve(
            this.app.models
                .WebPages(appId)
                .aggregate([{ $match: pagequery }, { $group: { _id: null, amount: { $sum: '$total_res_size' } } }])
                .read('sp')
                .exec()
        );
        const ajaxflow = Promise.resolve(
            this.app.models
                .WebAjaxs(appId)
                .aggregate([{ $match: querydata }, { $group: { _id: null, amount: { $sum: '$decoded_body_size' } } }])
                .read('sp')
                .exec()
        );
