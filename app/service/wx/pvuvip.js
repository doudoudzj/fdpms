...
class PvuvipService extends Service {
    ...
    async getHistoryPvUvIplist(appId) {
        ...
        return await this.ctx.model.Wx.WxPvuvip.find(query).read('sp').sort({ create_time: -1 }).limit(5).exec();
    }
    /* eslint-disable */
    // 概况统计
    ...
    async flow(appId, querydata) {
        return this.app.models
            .WxAjaxs(appId)
            .aggregate([{ $match: querydata }, { $group: { _id: null, amount: { $sum: '$body_size' } } }])
            .read('sp')
            .exec();
    }
